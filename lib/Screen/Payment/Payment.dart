...
class StatePayment extends State<Payment> with TickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Obtener crédito disponible al iniciar
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<CartProvider>().obtenerCreditoDisponible(context);
    });
  ...
  Widget paymentItem(int index) {
    final isCredit = context.read<PaymentProvider>().payModel[index].name == 'Credito';

    if (isCredit) {
      final createdAt = context.read<UserProvider>().createdAt;

      // Solo usuarios creados ANTES del 2026 pueden ver la opción de crédito
      // Usuarios creados a partir del 2026 no la ven
      if (createdAt == null || createdAt.year >= 2026) {

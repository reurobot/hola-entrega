...
class ProductStatesticsAnimatedContainer extends StatefulWidget {
  ...
  const ProductStatesticsAnimatedContainer(
      ...
      statisticsWidgets = [
        if (widget.statistics.totalInCart != '0')
          _itemWidget(
              ...
              text:
                  " ${'PEOPLE_HAVE_ADDED_IN_CART'.translate(context: context)}"),
        if (widget.statistics.totalFavourites != '0')
          _itemWidget(
              icon: Icons.favorite,
              count: widget.statistics.totalFavourites,
              text:
                  " ${'PEOPLE_HAVE_ADDED_IN_FAVOURITES'.translate(context: context)}"),
        if (widget.statistics.totalOrders != '0')
          _itemWidget(
              icon: Icons.shopping_cart_rounded,
              count: widget.statistics.totalOrders,
              text:
                  " ${'ITEMS_SOLD_IN_LAST_30_DAYS'.translate(context: context)}"),
      ];
      setState(() {
        loading = false;
      });

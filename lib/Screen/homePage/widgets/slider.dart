...
import 'package:eshop_multivendor/Screen/SliderProductList/slider_product_list.dart';
    ...
    Future.delayed(const Duration(seconds: 10)).then(
      (_) {
        if (mounted) {
          ...
          if (nextPage == context.read<HomePageProvider>().homeSliderList.length) {
            nextPage = 0;
          }
  ...
  Widget buildImagePageItem(Model slider) {
    ...
    return GestureDetector(
      ...
      onTap: () async {
        ...
        if (context.read<HomePageProvider>().homeSliderList[curSlider].type == 'products') {
          Navigator.push(
            context,
            PageRouteBuilder(
              pageBuilder: (_, __, ___) => SliderProductList(
                id: context.read<HomePageProvider>().homeSliderList[curSlider].id ?? '',
                name: context.read<HomePageProvider>().homeSliderList[curSlider].name ??
                    'Slider product list',
              ),
            ),
          );
        } else if (context.read<HomePageProvider>().homeSliderList[curSlider].type == 'categories') {
          Product item = context.read<HomePageProvider>().homeSliderList[curSlider].list as Product;
        ...
        } else if (context.read<HomePageProvider>().homeSliderList[curSlider].type == 'slider_url') {
          String url = context.read<HomePageProvider>().homeSliderList[curSlider].urlLink.toString();
          try {
